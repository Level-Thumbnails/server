pub const PENDING_UPLOAD_SELECT: &str = "SELECT uploads.id, user_id, users.username AS username, level_id, accepted, upload_time, accepted_time, users.account_id AS account_id, users.discord_id AS discord_id, users.role AS user_role, \
